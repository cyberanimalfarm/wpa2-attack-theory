...
In WPA2 (as well as in WPA), the GTK, or Group Temporal Key, plays a crucial role in securing multicast and broadcast traffic on the wireless network. 

Here's a breakdown:

1. **Multicast/Broadcast vs. Unicast Traffic**: In a Wi-Fi network, there are typically two types of traffic:
   - **Unicast**: Traffic that's meant for one specific device. It's transmitted from one device directly to another. This traffic is encrypted using a pairwise key, namely the Pairwise Transient Key (PTK), which is unique for each client-device association.
   - **Multicast/Broadcast**: Traffic that's meant for multiple devices or all devices on the network. Examples include ARP requests or certain DHCP messages. This traffic needs to be encrypted in such a way that all legitimate devices on the network can decrypt it.

2. **Role of GTK**: Given the nature of multicast/broadcast traffic, it's not feasible to encrypt this traffic with the unique PTK of every device on the network. Instead, a common key is used for this purpose, which is the GTK. Every device on the network is provided the GTK so that they can decrypt multicast and broadcast traffic.

3. **Distribution in the Four-Way Handshake**: 
   - The GTK is generated by the Access Point (AP).
   - During the four-way handshake, once the client and the AP have established mutual trust and have derived the PTK, the AP sends the GTK to the client.
   - To secure this transmission, the GTK is encrypted using a key derived from the PTK, ensuring that only the client can decrypt and obtain the GTK.
   - Specifically, the GTK is sent in the third message of the four-way handshake from the AP to the client.

4. **Rekeying**: For enhanced security, the GTK can be periodically updated (rekeyed). When this happens, the new GTK needs to be distributed to all associated clients. This process is typically achieved through a group key handshake, which is a separate process from the four-way handshake.

In summary, the GTK in the WPA2 protocol ensures that multicast and broadcast traffic on a Wi-Fi network is encrypted and only accessible by authorized devices.
